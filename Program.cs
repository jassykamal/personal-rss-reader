...
static void MigrateDatabase(AppDbContext db)
{
    ...
    try
    {
        ...
        db.Database.ExecuteSqlRaw(
            "CREATE UNIQUE INDEX IF NOT EXISTS IX_ArticleSummaries_ArticleUrl ON ArticleSummaries (ArticleUrl)");
    }
...
app.MapPost("/api/ai/summarize", async (
    ...
    AppDbContext db) =>
{
    if (string.IsNullOrWhiteSpace(req.ArticleUrl))
        return Results.BadRequest(new { error = "Article URL is required." });

    var existing = await db.ArticleSummaries
        .FirstOrDefaultAsync(s => s.ArticleUrl == req.ArticleUrl);

    if (existing != null)
        return Results.Ok(new { summary = existing.Summary, cached = true });

    var summary = "This is a placeholder AI summary for development. " +
                  "It will be replaced with an actual summary generated by the AI model. " +
                  "The article titled \"" + (req.Title ?? "Unknown") + "\" " +
                  "from " + (req.FeedTitle ?? "unknown source") + " " +
                  "would be analyzed and summarized here.";

    db.ArticleSummaries.Add(new ArticleSummary
    {
        ArticleUrl = req.ArticleUrl,
        Summary = summary,
    ...
    return Results.Ok(new { summary, cached = false });
}).DisableAntiforgery();
