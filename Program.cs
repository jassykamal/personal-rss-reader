    ...
    IDeepSeekService deepSeek) =>
{
    ...
    try
    {
        summary = await deepSeek.SummarizeAsync(
            req.Title, req.Description, req.FeedTitle, req.ContentType ?? "News");
    }
    catch (InvalidOperationException)
    {
        return Results.BadRequest(new { error = "AI summarization is not configured on this server." });
    }
